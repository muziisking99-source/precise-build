...
function SettingsAdmin() {
  ...
  const [goldBandBody, setGoldBandBody] = useState("11 ranges. Every flavour. Every South African table.");
  ...
  useEffect(() => {
    supabase.from("site_settings").select("key,value").in("key", [...SETTING_KEYS]).then(({ data }) => {
      (data ?? []).forEach((r: { key: string; value: string | null }) => {
        ...
        if (r.key === "gold_band_body") setGoldBandBody(r.value ?? "");
      });
    });
  }, []);
  ...
  const save = async () => {
    const rows = [
      ...
      { key: "gold_band_body", value: goldBandBody },
    ];
  ...
  return (
    <AdminShell title="Site Settings">
      ...
      <div className="admin-card" style={{ maxWidth: 760 }}>
        ...
        <div className="admin-field">
          <label className="admin-label">Gold band body text</label>
          <textarea className="admin-textarea" rows={2} value={goldBandBody} onChange={(e) => setGoldBandBody(e.target.value)} />
        </div>
      </div>
